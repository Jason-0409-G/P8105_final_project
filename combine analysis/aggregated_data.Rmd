...
nhanes_period_clean <- nhanes_period %>%
  rename(
    survey_years = period,          # 统一周期名称
    nhanes_prev = prev_num,         # 更清晰的命名
    nhanes_measure = measure_clean  # 可选：记录 NHANES 指标
  ) %>%
  select(survey_years, age_group = age_group_baseline, nhanes_prev, nhanes_measure)
```
